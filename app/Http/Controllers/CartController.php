...
class CartController extends Controller
{
    ...
    public function update_show_cart(Request $request)
    {
        if (!Auth::check()) {
        ...
        return response()->json(['success' => true]); // Không cần trả về dữ liệu chi tiết theo yêu cầu
    }

    public function delete_from_show_cart(Request $request, $productId)
    {
        if (!Auth::check()) {
            return response()->json(['success' => false, 'message' => 'Vui lòng đăng nhập!'], 401);
        }
        ...
        return response()->json(['success' => true]); // Không cần trả về dữ liệu chi tiết
    }
}
